...
// tablice zawierające kąty padania swiatła dla obu źródeł światła 
static GLfloat fi[2] = { 5.76f, 1.05f };
static GLfloat theta[2] = { 4.68f, 4.68f };

// zamienne przechowywujące wartości kątów w pikselach
...
// liczba poziomów rysowania jajka
const int N = 40;

// zmeinna do wyznaczania wartosci v i u
float var = 1.0f / N;

float lightRadius = 10.0;

// typ potrzebny o definicji tablicy jajka
struct eggPoints {
	// współrzędne punktów
	float x;
	float y;
	...
	// współrzędne punktów do rysowania tekstury
	float l;
	float p;
};

//tablica przechowywująca punkty jajka
...
//funkcja wczytujaca obraz .tga do programu
...
void MakeTableOfPoints() {
	for (int i = 0; i < N; i++) {
		for (int j = 0; j < N; j++) {
			// wartości u i v oraz długości wektora Normalnego
			float u = i * var;
			float v = j * var;
			float dlugosc_wektora;

			//obliczenie położenia punktów ze wzorów
			TableOfPoint[i][j].x = ((-90 * pow(u, 5) + 225 * pow(u, 4) - 270 * pow(u, 3) + 180 * pow(u, 2) - 45 * u) * cos(Pi * v));
			TableOfPoint[i][j].y = (160 * pow(u, 4) - 320 * pow(u, 3) + 160 * pow(u, 2));
			TableOfPoint[i][j].z = ((-90 * pow(u, 5) + 225 * pow(u, 4) - 270 * pow(u, 3) + 180 * pow(u, 2) - 45 * u) * sin(Pi * v));

			//obliczenie składowych wektora normalnego ze wzorów
			...
			// przypisanie punktów tekstury
			TableOfPoint[i][j].l = i * var;
			TableOfPoint[i][j].p = j * var;
		}
	}
}
...
void DrawAsColors() {
	//rozpoczecie rysowania trojkatow
	...
	for (int i = N / 2; i < N; i++) {
		int help = N - i;
		for (int j = 0; j < N - 1; j++) {
			// dla ostatnie przejscie petli musi połączyć punkty koncowe z poczatkowymi
			if (i == N - 1) {
				glNormal3f(-1 * TableOfPoint[i][j].normal_x, -1 * TableOfPoint[i][j].normal_y, -1 * TableOfPoint[i][j].normal_z);
				glTexCoord2f(-TableOfPoint[i][j].l, -TableOfPoint[i][j].p);
				...
				glTexCoord2f(-TableOfPoint[i][j+1].l, -TableOfPoint[i][j+1].p);
				...
				glTexCoord2f(-TableOfPoint[0][0].l, -TableOfPoint[0][0].p);
			...
			//w przeciwnym wypadku kolorujemy normalnie
				...
				glTexCoord2f(-TableOfPoint[i+1][j].l, -TableOfPoint[i+1][j].p);
				...
				glVertex3f(TableOfPoint[i][j + 1].x, TableOfPoint[i][j + 1].y - 5, TableOfPoint[i][j + 1].z);
				glNormal3f(-1 * TableOfPoint[i + 1][j + 1].normal_x, -1 * TableOfPoint[i + 1][j + 1].normal_y, -1 * TableOfPoint[i + 1][j + 1].normal_z);
				glTexCoord2f(-TableOfPoint[i+1][j+1].l, -TableOfPoint[i+1][j+1].p);
				glVertex3f(TableOfPoint[i + 1][j + 1].x, TableOfPoint[i + 1][j + 1].y - 5, TableOfPoint[i + 1][j + 1].z);
			}
		}
	}

	//polaczenie punktow na krancach jajka góra i dół
	glNormal3f(-1 * TableOfPoint[0][0].normal_x, -1 * TableOfPoint[0][0].normal_y, -1 * TableOfPoint[0][0].normal_z);
	glTexCoord2f(-TableOfPoint[0][0].l, -TableOfPoint[0][0].p);
	glVertex3f(TableOfPoint[0][0].x, TableOfPoint[0][0].y - 5, TableOfPoint[0][0].z);
	glNormal3f(-1 * TableOfPoint[N - 1][0].normal_x, -1 * TableOfPoint[N - 1][0].normal_y, -1 * TableOfPoint[N - 1][0].normal_z);
	glTexCoord2f(-TableOfPoint[N - 1][0].l, -TableOfPoint[N - 1][0].p);
	...
	glTexCoord2f(TableOfPoint[1][N - 1].l, TableOfPoint[1][N - 1].p);
	...
	glTexCoord2f(TableOfPoint[1][0].l, TableOfPoint[1][0].p);
	...
	glTexCoord2f(-TableOfPoint[N - 1][N - 1].l, -TableOfPoint[N - 1][N - 1].p);
	glVertex3f(TableOfPoint[N - 1][N - 1].x, TableOfPoint[N - 1][N - 1].y - 5, TableOfPoint[N - 1][N - 1].z);

	// lustrzane odbicie tamtej strony z przeciwnymi wektorami normalnymi aby swiatlo zawsze bylo odbijane na zewn
...
//funkcja śledząca przyciski myszy
...
void MyInit(void)
{
	// zmienne przechowywujace dane tektury
	GLbyte* pBytes;
	...
	//wczytanie pliku zombie.tga do pBytes
	pBytes = LoadTGAImage("zombi.tga", &ImWidth, &ImHeight, &ImComponents, &ImFormat);

	//ustawienie pBytes na texture rysowana
	glTexImage2D(GL_TEXTURE_2D, 0, ImComponents, ImWidth, ImHeight, 0, ImFormat, GL_UNSIGNED_BYTE, pBytes);

	//zwolnienie miejsca z pBytes po wczytaniu
	free(pBytes);

	//wlaczenie trybu teksturowania
...
//funkcja utrzymujaca proporcje obrazu w razie powiekszania okna
